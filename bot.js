...
client.on("messageUpdate", msg => {
 ...
 const i = db.fetch(`${msg.guild.id}.kufur`)
    if (i) {
        const kufur = ["oç", "amk", "ananı sikiyim","piç","orospu çocuğu","orospu","oruspu","anan","yarak","yarrak"];
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea'|| msg.content.toLowerCase() == 'selam') {
          try {
                  ...
                  return msg.reply('Aleyküm Selam Kankam, Hoşgeldin')
          } catch(err) {
            console.log(err);
          }
      }
...
client.on("ready", () => {
  ...
  client.user.setActivity('!pyardım | Beta v1 | !pdavet');
})
